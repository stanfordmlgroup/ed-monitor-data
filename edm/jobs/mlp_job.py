...
class MlpJob():
    ...
    def __preprocess(self):
        ...
        if len(self.additional_cols) > 0:
            df_train_x, df_val_x, df_test_x = clean_additional_columns(df_train_x, df_val_x, df_test_x, cols_to_clean=self.additional_cols, ordinal_cols=self.ordinal_cols, impute=self.impute, normalize=self.normalize, save_path=self.save_predictions_path)
            if self.verbose >= 1:
