...
def process_patient(input_args):
    ...
    try:
        with h5py.File(filename, "r") as f:
            ...
            if abs(len(ii_processed) - len(ppg_processed)) > 0:
                # Skip due to rounding error due to precision
                print(f"[WARN] Skipping patient {csn} due to rounding error")
                return None
        ...
        for future in futures.as_completed(fs):
            ...
            if output is not None:
                ...
                try:
                    ...
                    waveform_start_time = datetime.strptime(waveform_start, "%Y-%m-%d %H:%M:%S.%f%z").replace(microsecond=0)
